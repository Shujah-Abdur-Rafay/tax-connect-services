...
import { createGigOrder, submitOrderIntake } from '@/services/gigOrdersService';
...
const ProjectBriefModal: React.FC<ProjectBriefModalProps> = ({ open, onClose, gig, tier }) => {
  ...
  const reset = () => {
    ...
    setShowIntake(false); setIntakeDone(false);
    ...
    try {
      ...
      if (user && gig && tier) {
        try {
          ...
          setOrderId(order.id);
          // Immediately prompt the post-purchase intake / order requirements.
          setShowIntake(true);
        } catch (orderErr: any) {
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => { if (!v) handleClose(); }}>
              ...
              <div className="mt-5 rounded-lg border border-emerald-200 bg-emerald-50 p-4 text-left">
                {intakeDone ? (
                  <p className="flex items-center gap-2 text-sm font-medium text-emerald-800">
                    <CheckCircle2 className="h-4 w-4" /> Order requirements submitted — your pro has what they need.
                  </p>
                ) : (
                  <>
                    <p className="text-sm font-semibold text-emerald-900">One quick step left</p>
                    <p className="mt-0.5 text-xs text-emerald-800">
                      Complete your order requirements so {gig.proName.split(',')[0]} can contact you and get started.
                    </p>
                    <Button
      ...
      {/* Post-purchase client intake / order requirements */}
      {orderId && user && (
        <ClientIntakeModal
          open={showIntake}
          ...
          onSubmit={async (intake) => {
            await submitOrderIntake(orderId, user.uid, intake);
            setIntakeDone(true);
          }}
        />
      )}
