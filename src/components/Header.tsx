...
const Header: React.FC<HeaderProps> = ({ onToggleNotifications, openLoginDialog = false }) => {
  const { user, logout, isAdmin, isProfessional, canAccessAdmin } = useAuth();
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16">
          ...
          <nav className="hidden md:flex space-x-7">
            ...
            {/* Member Portal is for members; admins use the dedicated console. */}
            {!isAdmin && (
          ...
          <div className="flex items-center space-x-4">
            {user ? (
              <>
                {/* Prominent dark entry to the dedicated admin console — only
                    for admins / help-desk staff. */}
                {canAccessAdmin && (
                  <Button
                ...
                <DropdownMenu>
                  ...
                  >
                    ...
                    {/* Admins live in the dedicated console, not the member
                        portal — hide the member-portal entry for full admins. */}
                    {!isAdmin && (
                    ...
                    {canAccessAdmin && (
                      <>
                        <DropdownMenuItem onClick={() => navigate('/admin')}>
                          ...
                          <span>{isAdmin ? 'Admin Panel' : 'Help Desk'}</span>
                        </DropdownMenuItem>
                        {isAdmin && (
