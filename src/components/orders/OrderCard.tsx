...
interface OrderCardProps {
  ...
  /**
   * Client-only: submit / update the post-purchase intake (order requirements).
   * Implementations typically call gigOrdersService.submitOrderIntake(...).
   */
  onSubmitIntake?: (orderId: string, intake: Omit<GigOrderIntake, 'submittedAt'>) => Promise<void>;
}
...
const OrderCard: React.FC<OrderCardProps> = ({
  ...
  const intakeSubmitted = !!intake?.submittedAt || !!order.intake_submitted_at;
  const clientCanSubmitIntake =
    perspective === 'client' && !isRefunded && order.status !== 'cancelled' && !!onSubmitIntake;
  // Hard gate: the client must complete the post-purchase intake (order
  // requirements) before the order can proceed (i.e. before paying / starting
  // work). Until then the Pay action opens the intake form instead.
  const mustCompleteIntakeFirst =
    clientCanSubmitIntake &&
    !intakeSubmitted &&
  ...
  return (
    <Card className={`overflow-hidden ${isRefunded ? 'border-red-200' : ''}`}>
      <div className="flex flex-col gap-4 p-4 md:flex-row">
        ...
        <div className="min-w-0 flex-1">
          <div className="mb-1 flex flex-wrap items-center gap-2">
            ...
            {/* Post-purchase intake / order requirements status */}
            {intakeSubmitted ? (
              <Badge
                variant="outline"
                className="bg-emerald-50 text-emerald-700 border-emerald-200 text-xs inline-flex items-center gap-1"
              ...
              <Badge
                variant="outline"
                className="bg-amber-50 text-amber-700 border-amber-200 text-xs inline-flex items-center gap-1"
                title="Waiting on the client to submit their requirements"
              >
                <ClipboardList className="h-3 w-3" /> Awaiting requirements
              </Badge>
            ) : null}
        ...
        {/* Required-intake banner — the order can't proceed until the client
            completes their order requirements. */}
        {mustCompleteIntakeFirst && (
          <div className="mb-3 flex items-start gap-2 rounded-lg border border-amber-300 bg-amber-50 px-3 py-2.5 text-sm text-amber-900">
            <ClipboardList className="mt-0.5 h-4 w-4 flex-shrink-0 text-amber-600" />
            <div className="flex-1">
              <p className="font-semibold">Order requirements needed</p>
              <p className="text-amber-800">
                Complete the short intake form so your tax pro can begin. You can’t proceed to
                payment until it’s submitted.
              </p>
            </div>
            <Button
              size="sm"
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {!isRefunded &&
            ...
            order.checkout_url && (
              <Button
                size="sm"
                onClick={() => {
                  if (mustCompleteIntakeFirst) {
                    toast({
                      title: 'Complete your order requirements first',
                      description:
                        'Your tax pro needs your intake details before you proceed to payment.',
                    });
                    setShowIntake(true);
                    return;
                  }
              ...
              >
                {mustCompleteIntakeFirst ? (
                  <>
                    <ClipboardList className="mr-1 h-3 w-3" /> Complete requirements to pay
                  </>
                ) : (
                  <>
          ...
          {/* Client: submit / edit post-purchase order requirements */}
          {clientCanSubmitIntake && (
            <Button
              size="sm"
              onClick={() => setShowIntake(true)}
              disabled={!!busy}
              variant={intakeSubmitted ? 'outline' : 'default'}
              className={
                intakeSubmitted
            ...
            >
              ...
              {intakeSubmitted ? 'Edit requirements' : 'Complete requirements'}
            </Button>
          )}
              ...
              {intake && intakeSubmitted ? (
                <div className="rounded-md border border-emerald-200 bg-emerald-50/60 p-3 text-sm">
                  <ul className="space-y-1 text-gray-700">
                    ...
                    <li><strong>Claiming dependents:</strong> {intake.claimingDependents === 'yes' ? 'Yes' : 'No'}</li>
                    <li><strong>Self-employed / business:</strong> {intake.selfEmployed === 'yes' ? 'Yes' : 'No'}</li>
                    <li><strong>Service needed:</strong> {intake.serviceNeeded}</li>
                    <li><strong>Best time to call:</strong> {intake.bestTimeToCall || '—'}</li>
                    {intake.additionalInfo && (
                      <li className="pt-1">
                        <strong>Notes for pro:</strong>{' '}
                        <span className="text-gray-600">{intake.additionalInfo}</span>
                      </li>
                    )}
                  </ul>
                </div>
              ) : (
                <div className="rounded-md border border-dashed border-amber-300 bg-amber-50/60 p-3 text-sm text-amber-800">
                  {perspective === 'client' ? (
                    <div className="flex flex-wrap items-center justify-between gap-2">
                      <span>You haven’t submitted your order requirements yet.</span>
                      {clientCanSubmitIntake && (
                        <Button
                          size="sm"
                          className="bg-emerald-600 hover:bg-emerald-700"
                          onClick={() => setShowIntake(true)}
                        >
                          <ClipboardList className="mr-1 h-3 w-3" /> Complete now
                        </Button>
                      )}
                    </div>
                  ) : (
                    <span>Waiting on the client to submit their order requirements (name, phone, email and screening questions).</span>
                  )}
                </div>
              )}
            </div>
      ...
      {/* Client: submit / edit post-purchase order requirements */}
      {clientCanSubmitIntake && onSubmitIntake && (
        <ClientIntakeModal
          open={showIntake}
          onClose={() => setShowIntake(false)}
          proName={order.pro_name}
          defaults={
            intake || {
          ...
          onSubmit={(data) => onSubmitIntake(order.id, data)}
        />
      )}
    </Card>
  );
