...
import { Search, Eye, Filter, Loader2, Users, FileText, Crown, CreditCard, Star, TrendingUp, Wrench, FolderOpen, Shield } from 'lucide-react';
...
// Map each dashboard tab to the permission that unlocks it. Full admins have
// every permission; help-desk users see only the tabs they were granted.
const TAB_PERMISSION: Record<string, AdminPermission> = {
...
export function AdminDashboard() {
  ...
  const can = (tab: string) => hasAdminPermission(TAB_PERMISSION[tab]);
  // First tab the current user can actually see (full admins → 'applications').
  const defaultTab =
    ['applications', 'professionals', 'memberships', 'subscriptions', 'payouts', 'reviews', 'content', 'admins', 'tools'].find(
      (t) => can(t),
    ) || 'applications';
