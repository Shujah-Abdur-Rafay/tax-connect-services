    ...
    <Card className="h-full hover:shadow-lg transition-shadow duration-200 cursor-pointer"
      ...
      <CardContent className="p-6">
        ...
        <div className="flex gap-2" onClick={(e) => e.stopPropagation()}>
          ...
          />
          {/* Message button — opens the real contact form (creates a Firestore
              conversation viewable in the Member Portal), replacing the old
              mock chat window that showed fake canned replies. */}
          <ProfessionalContactForm
            professionalName={professional.name}
            professionalId={professional.id}
            professionalEmail={professional.email}
            trigger={
              <Button variant="outline" size="sm" title={`Message ${professional.name}`}>
