...
import MemberContentLibrary from '@/components/MemberContentLibrary';
import { User, FileText, MessageSquare, CreditCard, Settings, Receipt, DollarSign, Bell, Upload, Lock, Megaphone, BarChart3, LogOut, FolderOpen } from 'lucide-react';
...
const MemberPortal: React.FC = () => {
  ...
  // Admins belong in the dedicated admin console, not the member portal. Keep
  // the two experiences fully separate — bounce full admins to /admin.
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
          <TabsList
            ...
            style={{ gridTemplateColumns: `repeat(${isAdmin ? 12 : 10}, minmax(0, 1fr))` }}
          >
            <TabsTrigger value="dashboard"><User className="w-4 h-4 mr-2" /><span className="hidden sm:inline">Dashboard</span></TabsTrigger>
            <TabsTrigger value="resources"><FolderOpen className="w-4 h-4 mr-2" /><span className="hidden sm:inline">Resources</span></TabsTrigger>
          ...
          <TabsContent value="resources"><MemberContentLibrary /></TabsContent>
          {/* Analytics & Marketing are admin-only */}
          {isAdmin && (
            <>
