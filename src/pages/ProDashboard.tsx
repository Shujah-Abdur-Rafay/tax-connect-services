...
import { isServiceBureau, hasUpgradedAccess } from '@/constants/membershipLevels';
import {
  LayoutDashboard,
  UserCog,
  Inbox,
...
// `requiresSoftware: true` marks the full software-suite tabs (client
// management, messaging, document handling, earnings). Per the tier spec these
// are locked for the free Directory Listing tier and unlock at Professional
// Partner and above (hasUpgradedAccess). Free-tier pros keep the public-profile
// tabs (overview, profile, billing, reviews) and the Resource Center (whose
// individual items are tier-gated internally).
const TABS = [
  { value: 'overview', label: 'Overview', icon: LayoutDashboard },
  { value: 'profile', label: 'Profile & Services', icon: UserCog },
  { value: 'leads', label: 'Leads', icon: Inbox, requiresSoftware: true },
  { value: 'appointments', label: 'Appointments', icon: CalendarDays, requiresSoftware: true },
  { value: 'messages', label: 'Messages', icon: MessageSquare, requiresSoftware: true },
  { value: 'documents', label: 'Documents', icon: FolderOpen, requiresSoftware: true },
  { value: 'earnings', label: 'Earnings', icon: TrendingUp, requiresSoftware: true },
...
const ProDashboard: React.FC = () => {
  ...
  const levelStr = user.membershipLevel ? String(user.membershipLevel) : undefined;
  const userIsBureau = isServiceBureau(levelStr);
  // Free Directory Listing pros don't get the software suite; it unlocks at
  // Professional Partner and above. Locked tabs stay visible (with a lock) and
  // render an UpgradePrompt so the upgrade path is always discoverable.
  const hasSoftware = hasUpgradedAccess(levelStr);
  const isLocked = (t: (typeof TABS)[number]) =>
    'requiresSoftware' in t && t.requiresSoftware && !hasSoftware;
  const visibleTabs = TABS.filter(
    (t) => !('serviceBureauOnly' in t && t.serviceBureauOnly) || userIsBureau,
  );
  const softwareUpgrade = <UpgradePrompt feature="the full professional software suite" />;
      ...
      <main className="mx-auto w-full max-w-6xl flex-1 px-4 py-8">
        ...
        <Tabs value={activeTab} onValueChange={changeTab} className="space-y-6">
          <TabsList className="grid w-full grid-cols-3 sm:grid-cols-5 lg:flex lg:w-auto lg:flex-wrap">
            {visibleTabs.map((t) => (
              <TabsTrigger key={t.value} value={t.value} className="data-[state=active]:text-blue-700">
                {isLocked(t) ? <Lock className="h-4 w-4 lg:mr-2" /> : <t.icon className="h-4 w-4 lg:mr-2" />}
          ...
          <TabsContent value="leads">
            {hasSoftware ? <ProLeadsInbox professionalId={user.uid} /> : softwareUpgrade}
          </TabsContent>

          <TabsContent value="appointments">
            {hasSoftware ? <AppointmentsList professionalId={user.uid} /> : softwareUpgrade}
          </TabsContent>

          <TabsContent value="messages">
            {hasSoftware ? <MessagingCenter currentUserId={user.id} /> : softwareUpgrade}
          </TabsContent>

          <TabsContent value="documents">
            {hasSoftware ? <ProfessionalDocumentViewer /> : softwareUpgrade}
          </TabsContent>

          <TabsContent value="earnings">
            {hasSoftware ? <ProEarningsSummary professionalId={user.uid} /> : softwareUpgrade}
          </TabsContent>
