...
service firebase.storage {
  match /b/{bucket}/o {
    ...
    // Admin-managed content libraries: the Resource Center (resources/) and the
    // Content Management module (member-content/). The REAL access gate is the
    // backing Firestore collection (resources/ and member_content/ are
    // admin-only writes; member_content reads are category-gated). Storage rules
    // cannot read Firestore roles, so we allow signed-in read/write here and
    // rely on the admin-only upload UI + admin-only Firestore metadata. Download
    // URLs carry an access token.
    match /resources/{allPaths=**} {
      allow read: if request.auth != null;
      allow write: if request.auth != null
                   && request.resource.size < 50 * 1024 * 1024;  // Max 50MB
    }
    match /member-content/{allPaths=**} {
      allow read: if request.auth != null;
      allow write: if request.auth != null
                   && request.resource.size < 50 * 1024 * 1024;  // Max 50MB
    }
